...
DPVALUE=$(cat /tmp/msg$$| tail -n -1 | awk '{ print 100-$15 }')
rm /tmp/msg$$
...
USED=`free -m | head -2 | tail -1 | awk '{ print $3 }'`
...
for i in `df -P | egrep -v "Tivoli|Mounted|mksysbfs|DoOnceAIX|proc|SCM|ITM|livedum|livedumpp|aha|aixinstall|mnt|tsmupdate" | grep "%"| awk '{ sub(/%/,""); print $5 ","$6 }'`
do
   IFS=, 
   set $i
   usage=$1
